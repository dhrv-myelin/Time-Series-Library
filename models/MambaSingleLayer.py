...
from layers.Embed import PositionalEmbedding, DataEmbedding
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.task_name in ["classification"]:  # one class per one sequence sample
            ...
            nn.init.xavier_uniform_(self.out_layer[1].weight)  # pyright: ignore
    ...
    def forward(self, x_enc, x_mark_enc, x_dec=None, x_mark_dec=None, mask=None):
        ...
        if self.task_name in ["classification"]:
            ...
            logit_out *= x_mark_enc.unsqueeze(
                2
            )  # (B, L_in, C_out)  # Mask out the padded sequence for variable length data (e.g. JapaneseVowels)

            ### Compute attention weights for weighted sum of logit_out
            w_out = self.attn_weight(
                mamba_out
            )  # (B, L_in, D) -> (B, L_in, n_head) -> (B, L_in, 1)
